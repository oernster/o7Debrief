...
Because the gate uses `--cov-fail-under=100`, a run has two ways to fail and only one of them is a test failure. Coverage falling below the threshold fails the run while every test still passes, so pytest's usual summary line can read `687 passed` on a run that exited non-zero. The line is real; it is just not the whole answer.
