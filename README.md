...
**Linux ships as a Flatpak and the play path has been walked.** Each release publishes `o7debrief.flatpak` alongside the Windows setup program. On Ubuntu with Elite Dangerous under Proton, the Flatpak has been left watching through a real session and produced a debrief when the game closed, which then opened by itself in a browser on the host. So the parts that matter most are proven rather than intended: the sandbox reads the journal inside the prefix and the report reaches a real browser.

Starting at sign-in is proven there too: it was found broken in the Flatpak, fixed in this release and confirmed by ticking the setting and rebooting. What has not been exercised is narrower: launching o7 Debrief a second time to summon the window inside the sandbox and the case where Steam itself is installed as a Flatpak. [TECH_DEBT.md](TECH_DEBT.md) item 2 records exactly what has and has not been verified. Windows remains the platform every release is built and tested on.
...
Tick "start o7 Debrief in the background when I sign in" and o7 Debrief watches from session start, so quitting the game opens the debrief in your browser exactly as it does on Windows. On Linux that setting writes an XDG autostart entry rather than a registry value; everything else behaves the same. Two things are worth knowing about it in the Flatpak. It did not work before this release: the entry was written inside the sandbox's own configuration rather than the session's, so the box stayed ticked and nothing started. That is fixed and confirmed on a real reboot; if you had it on before, tick it off and on again after upgrading so the entry is rewritten where your session reads it. And a Flatpak install can never arrive with it already on, because turning it on is your choice to make rather than the installer's, so on Linux it is always one trip into Settings after installing.

Not every Linux desktop draws a system tray, so o7 Debrief does not assume one. A stock GNOME session draws none, Ubuntu ships an extension that does, KDE, XFCE, MATE, Cinnamon and LXQt each provide one their own way; any of them can have it switched off. Rather than guess from the distribution, o7 Debrief asks your desktop what it actually provides; it asks for a few seconds rather than once, because started at sign-in the app is up before the panel that would host the icon has finished loading, so asking too early would report no tray on a session that is about to have one.
...
- A home screen on a left-click of the tray icon: the live status, the two debrief actions and the reports generated this run, all in one place. A right-click opens the full tray menu. On a desktop that draws no tray the home screen opens by itself; launching o7 Debrief while it is already running opens that same home screen instead of starting a second copy, so the app is reachable whether or not there is a tray to put an icon in.
...
- A session log that says what you actually did. Every row states the specifics the journal recorded rather than repeating the name of its kind: an engineering roll names the blueprint, the grade, the module and the engineer who applied it, so an evening of two hundred rolls reads as the work it was instead of two hundred identical lines. Applying an experimental effect gets its own row naming the effect, because the journal reports it as a roll and it is not one. The wording of each row lives in the TOML taxonomy beside the rule that produces it, so changing how something reads is a config edit. A row whose journal entry lacks a field the wording needs falls back to naming its kind, because a sentence with a gap in it would read as a fact.
...
- A credit change that is the truth, not the takings. The headline sits your balance beside the session's real change, taken from the balances the journal states at each end, so a rebuy, an outfitting spree or a hold full of tritium counts against you exactly as it did in game. A session that ended poorer says so. Where the journal stated too few balances to measure a change, the change is left off the card rather than shown as a zero you would read as breaking even. It is not announced as missing either: naming one quantity the report could not measure invites naming all of them. What the session's events did come to is on the card beside it.
- A balance that says when it was read. The journal states your balance at a login and never again, so on a long session the headline is what you had when you sat down rather than what you have now. The reading is real and it is what the journal said; the report prints the moment it was taken beneath it so you cannot mistake a figure hours old for a current one. There is no way to derive the closing balance instead: measured across a real journal, applying every priced event to the login reading matched the next login's stated balance on only two of fifty-four active sessions and was out by a median of eight million credits; four sessions moved the balance with no priced event of any kind in them.
- What the priced events came to, beside the balance rather than inside it. When the journal states one balance it can state no change; saying so and stopping there is true but useless. So the headline also carries the total of every event the journal does price, income less outgoings, in a card of its own that says plainly it is not the balance change. On a session that sold eighty stored modules that figure is +2,017,557,830 Cr, which is the figure worth having on a session whose balance change the journal never stated.
- Outfitting and the shipyard are accounted for. Modules and ships bought and sold are counted and priced on each side separately, so a refit that spent six million on new modules and took back nearly two billion for the old ones reads as what it was rather than as a single net figure. Ship transfers carry their own pair of lines, since a transfer buys nothing and still costs real money. Each purchase, sale and transfer is a row in the log naming the item and its price. The section states the two things the journal will not let it count: a Vessel Hangar bay, which is counted under Ship-Launched Vessel; and a ship taken in part-exchange, for which the journal states no sale.
- Engineering counts rolls and experimentals apart. A grade is rolled over and over for one module; an experimental effect is chosen once and applied once. The journal reports both as the same event, so counted together a session spent picking four effects and making thirty rolls read as thirty-four modifications.
- Readings the journal never gave are said out loud rather than shown as zero. A rank percentage carries its last known reading forward and reports "No reading" only when nothing is known at all, then the systems figure names where you actually are because a commander is always somewhere. Spending is shown too: the Trade section totals what your commodity purchases actually cost and the Outfitting and Shipyard section what your modules, ships and transfers did, since the journal states each. A count of nothing is never printed where the truth is that nothing was read.
...
The coverage flags live in `pyproject.toml`, so `pytest` alone applies the gate to the packages it covers. Do not add a bare `--cov`: it widens measurement to the UI and the composition roots, which are deliberately outside the gate; the run then fails the 100% threshold on a clean tree.
