...
The bundle it produces is what each release publishes as `o7debrief.flatpak`. It has been built, installed and run on Ubuntu. What has not been proven is producing a debrief from a real journal inside a Proton prefix, so [TECH_DEBT.md](TECH_DEBT.md) item 2 records exactly what has and has not been verified and is worth reading before you trust it.
