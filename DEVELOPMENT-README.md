...
The bundle it produces is what each release publishes as `o7debrief.flatpak`. It has been built, installed and used through a real play session on Ubuntu, with Elite Dangerous under Proton: the journal was read from inside the prefix and the debrief opened in a browser on the host. [TECH_DEBT.md](TECH_DEBT.md) item 2 records the narrower assumptions still unproven (the autostart entry across a session start, the summon route inside the sandbox and a Steam that is itself a Flatpak) and is worth reading before you trust those.
