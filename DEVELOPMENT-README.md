...
The bundle it produces is what each release publishes as `o7debrief.flatpak`. It has been built, installed and used through a real play session on Ubuntu, with Elite Dangerous under Proton: the journal was read from inside the prefix and the debrief opened in a browser on the host. [TECH_DEBT.md](TECH_DEBT.md) item 2 records what is left: the summon route inside the sandbox and a Steam that is itself a Flatpak are both untried, while the autostart entry was tried, found broken under the sandbox and fixed, so it now needs confirming on a real reboot rather than exercising for the first time.
