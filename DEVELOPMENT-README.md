...
The bundle it produces is what each release publishes as `o7debrief.flatpak`. It has been built, installed and used through a real play session on Ubuntu, with Elite Dangerous under Proton: the journal was read from inside the prefix and the debrief opened in a browser on the host. Starting at sign-in is proven there as well, after a defect that wrote the autostart entry inside the sandbox rather than into the session. [TECH_DEBT.md](TECH_DEBT.md) item 2 records what is left, which is two untried assumptions: the summon route inside the sandbox and a Steam that is itself a Flatpak.
