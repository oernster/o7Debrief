...
A sweep of the tree finds **88** comma breaches across **59 files** and no em dashes. The count is real rather than estimated: it comes from a detector that spans newlines. That matters because the common case is a comma ending one line and the conjunction opening the next; a single-line search misses every one of those and then reports a clean tree.
...
- **The `# noqa: E402` markers on the deferred imports.** The `tools/` scripts put the repository root on `sys.path` (and set the Qt scaling environment) before importing from the package, so the imports must follow the bootstrap; `tests/installer/test_worker_shutdown.py` does the same. The markers record that. `ruff check` reports thirty-eight unused-directive findings across the tree: the thirty-four E402 markers in `tools/`, the three in that installer test and one `# noqa: SLF001` on a capture script's deliberate private-attribute read. Every one is reported unused because ruff runs with those rules disabled and no project configuration to enable them; `--fix` would strip them and re-sort the imports. Under `flake8`, which is the linter the project configures, they are load-bearing and the tree is clean. This is a false positive from a tool the project does not use, not debt.
