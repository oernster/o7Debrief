...
Invariants I1 to I6 are the load-bearing structural rules and have dedicated tests. I7 to I11 are properties the structure makes possible and are pinned by the test suite rather than by a single AST scan.
...
- An exporter port, a configuration port and a release-source port that supplies the latest published release (version, releases page and installer assets) for the update check.
...
| A purchase states its cost on the magnitude channel, not the credit channel. | The journal states a purchase cost in `TotalCost`, so reporting nought spent beside ten purchases was a figure the journal contradicted. Routing it through the moment's magnitude keeps spending out of every credit total while still reading what was stated. | Giving a buy a `credits_field`, which would fold spending into income; leaving a stated cost unread. |
